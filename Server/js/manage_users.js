$(document).ready(function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('.alert_user').html('<p class="alert alert-success">Thêm sinh viên mới thành công</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('#dev_sel').val('0');
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-success">Dữ liệu sinh viên đã được cập nhật</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
  ...
  $(document).on('click', '.user_rmo', function(){
    ...
    bootbox.confirm("Bạn có muốn xoá dữ liệu của sinh viên này?", function(result) {
      if(result){
        $.ajax({
          url: 'manage_users_conf.php',
          type: 'POST',
          ...
          success: function(response){
            ...
            if (response == 1) {
              ...
              $('#dev_sel').val('0');
              $('.alert_user').fadeIn(500);
              $('.alert_user').html('<p class="alert alert-success">Dữ liệu đã được xoá thành công!</p>');
            }
            else{
              $('.alert_user').fadeIn(500);
              $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
  ...
  $(document).on('click', '.select_btn', function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        $('.alert_user').fadeIn(500);
        $('.alert_user').html('<p class="alert alert-success">Thẻ đã được chọn!</p>');
