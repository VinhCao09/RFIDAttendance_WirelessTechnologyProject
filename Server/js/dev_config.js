$(document).ready(function(){
		...
		$.ajax({
		  ...
		  success: function(response){
		    ...
		    if (response == 1) {			    
			    ...
			    $('.alert_dev').html('<p class="alert alert-success">Thêm dữ liệu thiết bị mới thành công</p>');
	...
	$(document).on('click', '.dev_uid_up', function(){
		...
		bootbox.confirm("Bạn chắc chắn muốn cập nhật lại ID cho thiết bị này?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
			       ...
			       success: function(response){
			        ...
			        if(response == 1){			    
				    	$('.alert_dev').fadeIn(500);
			        	$('.alert_dev').html('<p class="alert alert-success">Mã token đã được cập nhật</p>');
	...
	$(document).on('click', '.dev_del', function(){
		...
		bootbox.confirm("Bạn chắc chắn muốn xoá thiết bị này ko?", function(result) {
		if(result){
		     // AJAX Request
		     $.ajax({
		       url: 'dev_config.php',
	...
	$(document).on('click', '.mode_sel', function(){
		...
		bootbox.confirm("Bạn chắc chắn muốn thay đổi chế độ cho thiết bị này?", function(result) {
			if(result){
			     // AJAX Request
			     $.ajax({
			       url: 'dev_config.php',
